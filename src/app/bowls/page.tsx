...
export default function BowlsPage() {
  ...
  // Demo tournament ID — in production this would come from the route or admin selection
  const tournamentId = "demo-bowls-tournament";
  ...
  useEffect(() => {
    ...
    // Poll for check-in updates every 5 seconds for real-time feel
    const interval = setInterval(loadCheckins, 5000);
    return () => clearInterval(interval);
  }, [loadPlayers, loadCheckins]);

  function handlePlayerTap(player: Player) {
    // If already checked in, show them in the board
  ...
  // Position counts from check-ins
