...
export default function BrandingPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h1 className="text-2xl font-bold text-zinc-100">Venue Branding</h1>
      ...
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="https://..."
            className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2.5 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
          />
        </div>
          ...
          <div>
            ...
            <div className="flex gap-2 items-center">
              ...
              <input
                value={venue.primary_color || "#22c55e"}
                onChange={(e) => setVenue({ ...venue, primary_color: e.target.value })}
                className="flex-1 rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-100 font-mono"
              />
            </div>
          </div>
          <div>
        ...
        <div>
          <label className="block text-sm text-zinc-400 mb-1">Contact Phone</label>
          <input
      ...
      {/* Preview */}
      <div className="rounded-xl border border-zinc-700 p-6" style={{ backgroundColor: venue.secondary_color || "#0f172a" }}>
        <p className="text-xs text-zinc-500 mb-2">Preview</p>
        <div className="flex items-center gap-3">
          {venue.logo_url ? (
            <img src={venue.logo_url} alt="Logo" className="h-12 w-12 rounded-lg object-cover" />
          ) : (
            <div className="h-12 w-12 rounded-lg flex items-center justify-center text-white font-bold" style={{ backgroundColor: venue.primary_color || "#22c55e" }}>
              {venue.name?.charAt(0) || "V"}
            </div>
          )}
          <div>
            <h3 className="font-bold text-zinc-100">{venue.name}</h3>
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={save}
          disabled={isPending}
          className="rounded-lg bg-emerald-500 px-6 py-2.5 text-sm font-bold text-white hover:bg-emerald-400 transition-colors"
        >
