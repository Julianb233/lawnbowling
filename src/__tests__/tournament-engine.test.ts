...
describe("sortStandings", () => {
  ...
  it("sorts by wins first", () => {
    ...
    expect(sortStandings(standings).map((s) => s.playerId)).toEqual(["b", "c", "a"]);
  });
  ...
  it("breaks ties with shot difference", () => {
    ...
    expect(sortStandings(standings)[0].playerId).toBe("b");
  });
...
describe("buildStandings", () => {
  it("builds standings from rink results", () => {
    const results = [{
      teamAPlayerIds: ["p1", "p2"], teamBPlayerIds: ["p3", "p4"],
      totalA: 20, totalB: 15, endsWonA: 10, endsWonB: 5,
      winner: "team_a" as const, isFinalized: true,
    }];
  ...
  it("ignores non-finalized results", () => {
    const results = [{
      teamAPlayerIds: ["p1"], teamBPlayerIds: ["p2"],
      totalA: 10, totalB: 5, endsWonA: 5, endsWonB: 3,
      winner: "team_a" as const, isFinalized: false,
    }];
    expect(buildStandings(results)).toHaveLength(0);
  });

  it("accumulates across multiple rounds", () => {
    const results = [
      { teamAPlayerIds: ["p1"], teamBPlayerIds: ["p2"], totalA: 20, totalB: 15, endsWonA: 8, endsWonB: 6, winner: "team_a" as const, isFinalized: true },
      { teamAPlayerIds: ["p2"], teamBPlayerIds: ["p1"], totalA: 18, totalB: 12, endsWonA: 9, endsWonB: 5, winner: "team_a" as const, isFinalized: true },
    ];
    const standings = buildStandings(results);
    const p1 = standings.find((s) => s.playerId === "p1")!;
...
describe("generateSeededDraw", () => {
  it("pairs top seed with bottom seed (snake seeding)", () => {
    ...
    expect(pairings[0]).toEqual({ player1Id: "s1", player2Id: "s4", matchNumber: 1 });
    expect(pairings[1]).toEqual({ player1Id: "s2", player2Id: "s3", matchNumber: 2 });
  });
  ...
  it("handles 8, 16, and 32 players", () => {
    expect(generateSeededDraw(Array.from({ length: 8 }, (_, i) => `s${i}`))).toHaveLength(4);
    expect(generateSeededDraw(Array.from({ length: 16 }, (_, i) => `s${i}`))).toHaveLength(8);
    expect(generateSeededDraw(Array.from({ length: 32 }, (_, i) => `s${i}`))).toHaveLength(16);
  });
});
...
describe("handleBye", () => {
  it("returns player1 when player2 is null", () => {
    expect(handleBye({ player1Id: "p1", player2Id: null, matchNumber: 1 })).toBe("p1");
  });

  it("returns null when both players are present", () => {
    expect(handleBye({ player1Id: "p1", player2Id: "p2", matchNumber: 1 })).toBeNull();
  });
});
...
describe("generateSingleEliminationBracket", () => {
  it("generates correct bracket for 4 players", () => {
    ...
    expect(matches[0].player2Id).toBe("p4");
  });

  it("generates correct bracket for 8 players", () => {
    const matches = generateSingleEliminationBracket(Array.from({ length: 8 }, (_, i) => `p${i + 1}`));
  ...
  it("handles byes for 5 players", () => {
    const matches = generateSingleEliminationBracket(["p1", "p2", "p3", "p4", "p5"]);
    const byes = matches.filter((m) => m.round === 1 && m.status === "completed");
    expect(byes.length).toBeGreaterThan(0);
  });

  it("handles 16 and 32 players", () => {
    expect(generateSingleEliminationBracket(Array.from({ length: 16 }, (_, i) => `p${i}`)).filter((m) => m.round === 4)).toHaveLength(1);
    expect(generateSingleEliminationBracket(Array.from({ length: 32 }, (_, i) => `p${i}`)).filter((m) => m.round === 5)).toHaveLength(1);
  });
...
describe("generateDoubleEliminationBracket", () => {
  it("generates winners, losers, and grand final brackets", () => {
    const matches = generateDoubleEliminationBracket(["p1", "p2", "p3", "p4"]);
    expect(matches.filter((m) => m.bracket === "winners").length).toBeGreaterThan(0);
    expect(matches.filter((m) => m.bracket === "losers").length).toBeGreaterThan(0);
    expect(matches.filter((m) => m.bracket === "grand_final")).toHaveLength(1);
  });

  it("first round has correct seeding", () => {
    const matches = generateDoubleEliminationBracket(["p1", "p2", "p3", "p4"]);
...
describe("getNextMatchSlot", () => {
  it("match 1 goes to round 2 match 1 player1 slot", () => {
    expect(getNextMatchSlot(1, 1)).toEqual({ nextRound: 2, nextMatchNumber: 1, slot: "player1Id" });
  });

  it("match 2 goes to round 2 match 1 player2 slot", () => {
    expect(getNextMatchSlot(2, 1)).toEqual({ nextRound: 2, nextMatchNumber: 1, slot: "player2Id" });
  });

  it("match 3 goes to round 2 match 2 player1 slot", () => {
    expect(getNextMatchSlot(3, 1)).toEqual({ nextRound: 2, nextMatchNumber: 2, slot: "player1Id" });
  });
});
...
describe("isTournamentComplete", () => {
  it("returns true when all completed", () => {
    expect(isTournamentComplete([{ status: "completed" }, { status: "completed" }])).toBe(true);
  });

  it("returns false when some pending", () => {
    expect(isTournamentComplete([{ status: "completed" }, { status: "pending" }])).toBe(false);
  });
...
describe("determineTournamentWinner", () => {
  ...
  it("returns final match winner for single elimination", () => {
    ...
    expect(determineTournamentWinner(matches, "single_elimination")).toBe("p1");
  });
  ...
  it("returns standings leader for round robin", () => {
    ...
    expect(determineTournamentWinner([], "round_robin", standings)).toBe("p1");
  });
  ...
  it("returns grand final winner for double elimination", () => {
    ...
    expect(determineTournamentWinner(matches, "double_elimination")).toBe("p2");
  });
});
...
describe("handleForfeit", () => {
  it("awards win to other player", () => {
    expect(handleForfeit("p1", "p2", "p1")).toEqual({ winnerId: "p2", loserId: "p1" });
    expect(handleForfeit("p1", "p2", "p2")).toEqual({ winnerId: "p1", loserId: "p2" });
  });
});
