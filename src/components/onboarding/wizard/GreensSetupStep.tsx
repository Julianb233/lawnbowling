...
export function GreensSetupStep({ greens, onChange, errors }: GreensSetupStepProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-black text-[#0A2E12]">Greens Setup</h2>
        <p className="mt-1 text-sm text-[#3D5A3E]">Add your bowling greens and how many rinks each has.</p>
      </div>
      ...
      <div className="space-y-3">
        {greens.map((green, i) => (
          <div key={i} className="flex items-center gap-3 rounded-xl border border-[#0A2E12]/10 bg-white p-4">
            <div className="flex-1">
              <label className="mb-1 block text-xs font-medium text-[#3D5A3E]">Green Name</label>
              <input
                type="text"
                value={green.name}
            ...
            <div className="w-24">
              <label className="mb-1 block text-xs font-medium text-[#3D5A3E]">Rinks</label>
              <input
                type="number"
                min="1"
                max="20"
            ...
            {greens.length > 1 && (
              <button
                ...
                className="mt-5 rounded-lg p-2 text-[#3D5A3E] hover:bg-red-50 hover:text-red-500 transition-colors min-h-[44px]"
