...
export default function NextDrawSlide({ scores, maxRound }: NextDrawSlideProps) {
  ...
  return (
    <div className="flex h-full flex-col px-[clamp(1rem,2vw,2rem)]">
      <h2 className="mb-[clamp(0.5rem,1vh,1rem)] text-[clamp(1rem,2vw,1.5rem)] font-black uppercase tracking-wider text-[#3D5A3E]">
      ...
      <div className="grid flex-1 gap-[clamp(0.75rem,1.5vw,1.5rem)] md:grid-cols-2 xl:grid-cols-3">
        {nextRoundScores
          ...
          .map((s) => (
            <div key={s.id} className="rounded-2xl border border-white/10 bg-[#0A2E12] p-[clamp(1rem,2vw,1.5rem)]">
              <p className="mb-[clamp(0.5rem,1vh,0.75rem)] text-center text-[clamp(0.7rem,1vw,0.875rem)] font-bold uppercase tracking-wider text-[#3D5A3E]">
                Rink {s.rink}
              </p>
