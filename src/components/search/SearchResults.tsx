...
export function SearchResults({ players, teams, games, onSelect }: SearchResultsProps) {
  ...
  return (
    <div className="max-h-80 overflow-y-auto">
      {players.length > 0 && (
        <div>
          <p className="px-4 py-2 text-xs font-semibold uppercase tracking-wider text-zinc-500">Players</p>
          {players.map((p, i) => (
            <motion.button
              key={p.id}
      ...
      {teams.length > 0 && (
        <div>
          <p className="px-4 py-2 text-xs font-semibold uppercase tracking-wider text-zinc-500">Teams</p>
          {teams.map((t, i) => (
            <motion.button
              key={t.id}
              initial={{ opacity: 0, x: -10 }}
      ...
      {games.length > 0 && (
        <div>
          <p className="px-4 py-2 text-xs font-semibold uppercase tracking-wider text-zinc-500">Games</p>
          {games.map((g, i) => (
            <motion.button
              key={g.id}
              initial={{ opacity: 0, x: -10 }}
