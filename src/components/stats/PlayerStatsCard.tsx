...
export function PlayerStatsCard({ stats }: PlayerStatsCardProps) {
  if (!stats) {
    return (
      <div className="rounded-2xl border border-[#0A2E12]/10 bg-white/80 p-6 text-center">
        <Target className="mx-auto mb-2 h-10 w-10 text-[#2D4A30]" />
        <p className="text-sm text-[#3D5A3E]">No stats yet. Play some games!</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-[#0A2E12]/10 bg-white/80 p-6">
      ...
      <div className="mb-6 flex items-center justify-center">
        <div className="relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-2xl font-bold text-[#0A2E12]">{Math.round(stats.win_rate)}%</span>
      ...
      {/* Favorite sport */}
      {sportLabel && (
        <div className="mt-4 flex items-center justify-center gap-2 rounded-xl bg-[#0A2E12]/[0.03] py-2 text-sm text-[#3D5A3E]">
