...
export function RulesAccordion({ rules }: { rules: SportGuide["rules"] }) {
  return (
    <section id="rules">
      <h2 className="mb-4 text-2xl font-bold text-zinc-100">Key Rules</h2>
      <motion.div
      ...
      >
        <Accordion.Root type="multiple" className="space-y-2">
          {rules.map((rule, i) => (
            ...
            >
              <Accordion.Trigger className="flex w-full items-center justify-between px-4 py-3 text-left text-zinc-100 hover:bg-zinc-800/50 transition-colors min-h-[44px] group">
