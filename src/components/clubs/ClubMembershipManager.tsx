...
const ROLE_COLORS: Record<ClubRole, string> = {
  owner: "text-amber-600 bg-amber-50 dark:bg-amber-950 border-amber-200 dark:border-amber-800",
  admin: "text-purple-600 bg-purple-50 dark:bg-purple-950 border-purple-200 dark:border-purple-800",
  manager: "text-blue-600 bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800",
  member: "text-zinc-600 dark:text-zinc-400 bg-zinc-50 dark:bg-white/5 border-zinc-200 dark:border-white/10",
  visitor: "text-zinc-400 bg-zinc-50 dark:bg-white/5 border-zinc-200 dark:border-white/10",
};
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">Members</h2>
        {canManage && (
          <button
      ...
      <AnimatePresence>
        {showInvite && canManage && (
          ...
          >
            <div className="rounded-xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 p-4 space-y-3">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 min-h-[44px]"
                >
              ...
              {inviteCode && (
                <div className="flex items-center gap-2 rounded-lg bg-zinc-50 dark:bg-white/5 p-3">
                  <code className="flex-1 text-xs text-zinc-600 dark:text-zinc-400 truncate">
                    {window.location.origin}/clubs/join/{inviteCode}
                  </code>
                  <button
                    onClick={handleCopyInvite}
                    className="shrink-0 rounded-md p-2 hover:bg-zinc-200 dark:hover:bg-white/10 min-h-[44px] min-w-[44px] flex items-center justify-center"
                  >
                    {copied ? (
          ...
          <button
            ...
            className={cn(
              ...
              filter === f
                ...
                : "bg-zinc-100 dark:bg-white/5 text-zinc-500 dark:text-zinc-400 hover:text-zinc-700"
            )}
          >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 pl-9 pr-4 py-2.5 text-sm text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 min-h-[44px]"
        />
      </div>
        ...
        <div className="space-y-2">
          {filtered.map((m) => {
            ...
            return (
              ...
              >
                {/* Avatar */}
                <div className="h-10 w-10 shrink-0 overflow-hidden rounded-full bg-zinc-100 dark:bg-white/10">
                  {m.player?.avatar_url ? (
                  ...
                  <div className="flex items-center gap-1">
                    {isPending && (
                      <button
                        ...
                        className="rounded-lg bg-green-50 dark:bg-green-950 px-3 py-1.5 text-xs font-medium text-green-700 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900 min-h-[36px]"
                      >
                        Approve
                      </button>
                    )}
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="rounded-lg p-1.5 hover:bg-zinc-100 dark:hover:bg-white/10 min-h-[36px] min-w-[36px] flex items-center justify-center"
                      >
                        <ChevronDown className="h-4 w-4 text-zinc-400" />
                      </button>
                      {editingId === m.id && (
                        <div className="absolute right-0 top-full z-10 mt-1 w-40 rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-[#1a3d28] py-1 shadow-lg">
                          {assignableRoles.map((r) => (
                            <button
                              key={r}
                              onClick={() => handleUpdateRole(m.id, r)}
                              className={cn(
                                "block w-full px-3 py-2 text-left text-xs hover:bg-zinc-50 dark:hover:bg-white/10",
                                r === m.role ? "font-bold text-[#1B5E20]" : "text-zinc-600 dark:text-zinc-400"
                              )}
                            >
                              {CLUB_ROLE_LABELS[r].label}
                            </button>
                          ))}
                          <hr className="my-1 border-zinc-200 dark:border-white/10" />
                          <button
                            onClick={() => handleRemove(m.id)}
                            className="block w-full px-3 py-2 text-left text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-950"
                          >
                            Remove
                          </button>
                        </div>
